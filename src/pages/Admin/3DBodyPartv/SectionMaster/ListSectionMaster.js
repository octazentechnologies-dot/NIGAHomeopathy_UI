...
const ListSectionMaster = () => {
  ...
  const fetchPage = (page) => {
    dispatch(getAnatomySectionMastersList({ PageNumber: page, PageSize: pageSize }));
    setCurrentPage(page);
  };

  useEffect(() => {
    dispatch(getAnatomySectionMastersList({ PageNumber: currentPage, PageSize: pageSize }));
  }, [dispatch]);
                  ...
                  <Row className="g-4">
                    ...
                    <Col className="col-sm-auto">
                      <div className="d-inline-flex gap-2">
                        ...
                        <Link to="/admin/add3dsectionmaster">
                            ...
                            {items.length > 0 ? (
                              items.map((row, index) => (
                                <tr key={row.ThreeDBodyPartSectionMasterID ?? index}>
                                  <td>{row.ThreeDBodyPartSectionMasterID ?? row.ThreeDBodyPartSectionMasterID}</td>
                                  <td>{row.ThreeD_BodyPart_MeshKey_Name}</td>
                                  <td>{row.SectionName}</td>
                                  <td className="text-center">
                                    <div className="d-inline-flex gap-2">
                                      <Link
                                        to="/admin/edit3dsectionmaster"
