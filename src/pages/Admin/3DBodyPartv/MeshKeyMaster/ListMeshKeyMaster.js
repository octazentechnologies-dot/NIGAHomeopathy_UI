...
import {
  getMeshKeyMastersList,
  deleteMeshKeyMaster,
} from '../../../../slices/admin/3dbodypart/meshkeymaster/thunk';

const ListMeshKeyMaster = () => {
  ...
  const loading = useSelector((state) => state?.MeshKeyMaster?.loading || false);
  const items =
    useSelector((state) => state?.MeshKeyMaster?.meshKeyMasterList?.resultObject) || [];
  const totalPages =
    useSelector((state) => state?.MeshKeyMaster?.meshKeyMasterList?.totalPageCount) || 1;
  const totalRecords =
    useSelector((state) => state?.MeshKeyMaster?.meshKeyMasterList?.totalRecordCount) ||
    items.length ||
    0;

  const buildListRequest = (page = currentPage) => ({
    PageNumber: page,
    PageSize: pageSize,
  ...
  useEffect(() => {
    dispatch(getMeshKeyMastersList(buildListRequest(currentPage)));
  ...
  document.title = 'Mesh Key Master';
  return (
    <React.Fragment>
                ...
                <CardHeader className="border-0">
                  <div className="admin-list-toolbar d-flex align-items-center justify-content-between gap-2 flex-wrap w-100">
                    ...
                    <div className="admin-list-toolbar__actions d-flex align-items-center gap-2 flex-shrink-0 ms-auto">
                      ...
                      <Link to="/admin/addmeshkeymaster" className="d-inline-flex">
                          ...
                          {items.length > 0 ? (
                            ...
                            <tr>
                              <td colSpan={3} className="text-center text-muted py-4">
                                {searchQuery ? 'No mesh keys match your search' : 'No Mesh Keys Available'}
                              </td>
                            </tr>
                          )}
                        </tbody>
...
export default ListMeshKeyMaster;
