...
const EditSubQuestionGroup = () => {
  ...
  const initialSections = useMemo(() => {
    if (selectedSubQuestionGroup.sections?.length) {
      return selectedSubQuestionGroup.sections.map((s) => ({
        value: s.sectionId,
        label: s.sectionName,
      }));
    }
    if (selectedSubQuestionGroup.sectionIds?.length && SectionDDLOptions.length) {
      return SectionDDLOptions.filter((opt) =>
        selectedSubQuestionGroup.sectionIds.includes(opt.value)
      );
    }
    return [];
  }, [selectedSubQuestionGroup, SectionDDLOptions]);

  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      subQuestionGroupName: selectedSubQuestionGroup.questionSubGroupName || '',
...
export default EditSubQuestionGroup;
